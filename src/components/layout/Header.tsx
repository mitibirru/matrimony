      ...
      <div className="container flex h-16 sm:h-20 items-center justify-between">
        ...
        <Link href={session ? "/dashboard" : "/"} className="flex items-center space-x-2 sm:space-x-3 group">
          ...
          <div className="flex flex-col">
            <span className="text-lg sm:text-2xl font-extrabold tracking-tight text-primary">
              {siteConfig.name}
            </span>
            <span className="text-[9px] sm:text-[10px] uppercase tracking-widest text-muted-foreground font-semibold hidden xs:block">
              Telugu & Marathi
            </span>
          </div>
        </Link>
        ...
        <div className="md:hidden flex items-center space-x-2">
          <ThemeToggle />
          <MobileNav isLoggedIn={!!session} user={session?.user} />
        </div>
